...
export function PracticeMode({ allWords, learnedState }: PracticeModeProps) {
  ...
  const handleRemove = (e: React.MouseEvent) => {
    ...
    if (newSession.length === 0) {
      ...
      // Adjust index: if we removed the last card, wrap to 0
    ...
    <div className="relative max-w-lg mx-auto">
          ...
          {sessionWords.map((word, i) => (
            <div
              ...
              className={`h-full flex-1 transition-colors duration-300 ${
                i === currentIndex ? "bg-neutral-500" : isLearned(word.id) ? "bg-emerald-900/60" : "bg-rose-500/60"
      ...
      >
        ...
        <div className={`flex flex-col items-center transition-all duration-300 ${isFlipped ? "scale-75 -translate-y-12 opacity-40" : "scale-100 translate-y-0 opacity-100"}`}>
          ...
          {!isFlipped && (
            <p className="text-gray-600 text-sm mt-8">Tap to reveal · Hover for pinyin</p>
          )}
        </div>
      ...
      <button
        ...
        className="w-full mt-3 py-3 rounded-xl text-sm font-semibold text-gray-500 hover:text-emerald-400 bg-neutral-950 border border-neutral-800 hover:border-emerald-900/50 transition-all flex items-center justify-center gap-2"
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
        </svg>
      ...
      <div className="flex justify-center gap-1 mt-6">
        {sessionWords.map((w, i) => (
          <div key={w.id} className={`w-1.5 h-1.5 rounded-full transition-colors ${i === currentIndex ? "bg-neutral-500" : isLearned(w.id) ? "bg-emerald-800" : "bg-rose-500"}`} />
        ))}
      </div>
