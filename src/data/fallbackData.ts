...
/**
 * Build per-character pinyin from space-separated syllables.
...
function buildPinyinWords(
  ...
  // Split pinyin, filter out punctuation tokens
  const tokens = pinyin.split(/\s+/);
  const pinyinOnly = tokens.filter(
    (t) => !/^[。，！？、；：""''（）《》…—.,!?;:'"()\-\s]+$/.test(t)
  );
  ...
  for (const char of chars) {
    ...
    if (isPunct) {
      ...
      result.push({ char, pinyin: pinyinOnly[pIdx] || "" });
      pIdx++;
    }
  }
...
/**
 * Limit English definitions to at most 3 meanings.
 */
...
/**
 * Infer category from English definition.
 */
function inferCategory(english: string, hskLevel: number): string {
  const e = english.toLowerCase();
  if (/\b(i|you|he|she|it|we|they|this|that|who|what)\b/.test(e)) return "Pronouns";
  if (/\bto\s+\w+/.test(e)) return "Verbs";
  if (/\b(one|two|three|four|five|six|seven|eight|nine|ten|hundred|thousand|number|how many)\b/.test(e)) return "Numbers";
  if (/\b(day|week|month|year|time|hour|minute|today|tomorrow|yesterday|morning|afternoon|evening|night)\b/.test(e)) return "Time";
  if (/\b(father|mother|son|daughter|brother|sister|family|husband|wife|child|parent)\b/.test(e)) return "Family";
  if (/\b(eat|drink|food|fruit|rice|water|tea|coffee|bread|meat|vegetable)\b/.test(e)) return "Food & Drink";
  if (/\b(big|small|good|bad|hot|cold|new|old|long|short|fast|slow|high|low|many|few|expensive|cheap)\b/.test(e)) return "Adjectives";
  if (/\b(school|student|teacher|class|book|study|learn|read|write|test|exam)\b/.test(e)) return "Education";
  if (/\b(work|job|company|office|business|money|buy|sell|shop|store)\b/.test(e)) return "Work & Business";
  if (/\b(in|on|at|to|from|with|for|about|under|above|between|inside|outside|front|back|left|right)\b/.test(e)) return "Location";
  if (/\b(can|will|must|should|want|need|like|love|know|think|feel|hope)\b/.test(e)) return "Auxiliary & Modal";
  if (/\b(particle|used to|aspect|marker|question)\b/.test(e)) return "Particles";
  return hskLevel === 1 ? "HSK 1 General" : "HSK 2 General";
}

/**
 * Convert fallback data to VocabWord[] format.
 */
export function buildFallbackVocabulary(): VocabWord[] {
  return FALLBACK_DATA.map((word, index) => ({
    id: index + 1,
    hanzi: word.hanzi,
    pinyin: word.pinyin,
    english: limitEnglish(word.english),
    hskLevel: word.hsk_level as 1 | 2,
    category: inferCategory(word.english, word.hsk_level),
